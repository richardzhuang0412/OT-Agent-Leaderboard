...
-- This view returns ALL valid evaluation results from sandbox_jobs
...
WITH job_metrics AS (
  -- Pre-compute accuracy for all jobs with metrics
  SELECT
    sj.*,
...
ORDER BY a.name, m.name, COALESCE(b_canonical.name, b.name), jm.job_timestamp;
